...
void Model::init_model() {
    /* 模型初始化
       执行上下文（m_context）成功设置并完成模型的初始化后，在同一个程序生命周期内无需再次初始化
    */
...
bool Model::build_engine() {
    // 希望在build一个engine的时候就把一系列初始化全部做完，其中包括
    // build一个engine、创建一个context、创建推理所用的stream、创建推理所需要的device空间
    ...
    shared_ptr<Int8EntropyCalibrator> calibrator(new Int8EntropyCalibrator(
        64, 
        "/APP/cudaprj/deploy-yolov5-od/yolov5-calibration-and-infer/calibration/calibration_list_coco.txt", 
        "/APP/cudaprj/deploy-yolov5-od/yolov5-calibration-and-infer/calibration/calibration_table.txt",
        3 * 288 * 512, 288, 512));
...
bool Model::load_engine() {
    // load一个engine的时候就把一系列初始化全部做完，其中包括
    // deserialize一个engine、创建一个context、创建推理所用的stream、创建推理所需要的device空间
...
bool Model::enqueue_bindings() {
    //将输入和输出的数据绑定（bindings）提交给 TensorRT 的执行上下文（m_context）
    //并通过指定的 CUDA 流（m_stream）在 GPU 上执行推理任务
    m_timer->start_gpu();
